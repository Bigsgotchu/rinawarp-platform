...
export let env: EnvConfig = {
  ...
  USAGE_BATCH_INTERVAL: parseInt(
    process.env.USAGE_BATCH_INTERVAL || '5000',
    10
  ),
  USAGE_RETENTION: parseInt(
    process.env.USAGE_RETENTION || String(30 * 24 * 3600),
    10
  ),

  // Analytics & monitoring
  EVENT_RETENTION: parseInt(
    process.env.EVENT_RETENTION || String(7 * 24 * 3600),
    10
  ),
  SESSION_TIMEOUT: parseInt(
    process.env.SESSION_TIMEOUT || String(30 * 60 * 1000),
    10
  ),
  METRICS_RETENTION: parseInt(
    process.env.METRICS_RETENTION || String(24 * 3600),
    10
  ),
  ...
  STRICT_RATE_LIMITING:
    process.env.NODE_ENV === 'production' ||
    process.env.STRICT_RATE_LIMITING === 'true',
  ...
  REDIS_PREFIX: process.env.REDIS_PREFIX || 'warp:',
