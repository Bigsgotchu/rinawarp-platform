...
import {
  ...
  CommandPatternService
} from '../services/command';
import { SystemMonitorService } from '../services/monitoring';
import { WorkflowLearningService } from '../services/workflow';
import { UserProfileService } from '../services/user';
import { ErrorPatternService } from '../services/error';
...
export class CommandController {
  ...
  async predictNextCommands(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const recentHistory = (await HistoryService.getHistory(5)).map(
        h => h.command
      );
  ...
  async executeCommand(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const historyEntry = await HistoryService.addEntry(
        command.command,
        result
      );
      ...
      await Promise.all([
        ...
        WorkflowLearningService.recordCommand(
          command,
          result,
          context.currentDirectory
        ),
        UserProfileService.updateFromCommand(userId, command, result),
        CommandPatternService.recordCommandExecution(
          command,
          result,
          systemState
        ),
      ...
      if (result.error) {
        ...
        errorAnalysis = await ErrorPatternService.analyzeError(
          ...
          result.error,
          {
            ...
            projectContext: await WorkspaceContextService.getWorkspaceContext(
              context.currentDirectory
            ),
            systemState,
          }
      ...
      res.json({
        ...
        workflow: workflow
          ? {
              pattern: workflow,
              nextSteps: workflow.steps.slice(
                workflow.steps.findIndex(s => s.command === command.command) + 1
              ),
            }
          : null,
  ...
  async getCompletions(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const completions = await CommandCompletionService.getCompletions({
        ...
        cursorPosition:
          parseInt(cursorPosition as string) || (input as string).length,
