...
import CommandChainService from '../services/command';
...
export class GitController {
  ...
  async executeGit(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const { workspacePath, context, analysis } =
        await this.getOperationContext(req);
      ...
      const suggestions =
        await WorkspaceContextService.suggestActions(workspacePath);
