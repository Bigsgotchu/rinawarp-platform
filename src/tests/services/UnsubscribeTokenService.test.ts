...
describe('UnsubscribeTokenService', () => {
  ...
  describe('verifyToken', () => {
    it('verifies valid tokens correctly', () => {
      validTypes.forEach(type => {
        const token = UnsubscribeTokenService.generateToken(userId, type);
        const isValid = UnsubscribeTokenService.verifyToken(token, userId, type);
    ...
    it('rejects tokens with wrong user ID', () => {
      ...
      const isValid = UnsubscribeTokenService.verifyToken(token, 'wrong-user', 'daily');
      expect(isValid).toBe(false);
    });

    it('rejects tokens with wrong report type', () => {
      const token = UnsubscribeTokenService.generateToken(userId, 'daily');
      const isValid = UnsubscribeTokenService.verifyToken(token, userId, 'weekly');
      expect(isValid).toBe(false);
    });
    ...
    it('rejects tampered tokens', () => {
      ...
      const isValid = UnsubscribeTokenService.verifyToken(tamperedToken, userId, 'daily');
      expect(isValid).toBe(false);
    });

    it('rejects malformed tokens', () => {
      const isValid = UnsubscribeTokenService.verifyToken('invalid-token', userId, 'daily');
      expect(isValid).toBe(false);
    });
    ...
    it('rejects expired tokens', () => {
      ...
      const isValid = UnsubscribeTokenService.verifyToken(token, userId, 'daily');
      expect(isValid).toBe(false);
  ...
  describe('generateUnsubscribeUrl', () => {
    it('generates valid unsubscribe URLs', () => {
      validTypes.forEach(type => {
        const url = UnsubscribeTokenService.generateUnsubscribeUrl(userId, type);
        expect(url).toMatch(new RegExp(`^${config.appUrl}/api/analytics/unsubscribe`));
    ...
    it('generates URLs with valid tokens', () => {
      validTypes.forEach(type => {
        ...
        expect(token).toBeDefined();
        expect(UnsubscribeTokenService.verifyToken(token!, userId, type)).toBe(true);
      });
    });
  });
});
