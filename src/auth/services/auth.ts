...
import { AuthCredentials, AuthOptions, AuthState, AuthEvents, TokenStorage } from '../types';
...
export class AuthService {
  ...
  private async refreshAuth(refreshToken: string): Promise<void> {
    try {
      const auth = await TerminalApiClient.getInstance().refreshToken(refreshToken);
      this.setAuth(auth);
  ...
  // Event handlers
  public on<K extends keyof AuthEvents>(event: K, handler: AuthEvents[K]): void {
