...
Be specific and security-conscious in your suggestions.`;
  ...
  async analyzeCommand(
    ...
    try {
      ...
      if (context) {
        contextualPatterns = await CommandLearningService.getContextualSuggestions(command, context);
      }
      ...
      const alternatives: CommandSuggestion[] = this.extractSection(response, 'Alternatives')
        .split('\n')
        .map(alt => {
      ...
      const commandChains: CommandSuggestion[] = this.extractSection(response, 'Command Chains')
        .split('\n')
        .map(chain => {
    ...
    try {
      ...
      const completion = await this.openai.chat.completions.create({
        ...
        messages: [
          { 
            role: "system", 
            content: "You are a security expert. Respond with 'SAFE' or 'UNSAFE' followed by a brief reason. Consider rm -rf, curl | bash, and similar dangerous patterns." 
          },
...
Error: ${errorOutput}`
      ...
      const suggestedFixes: CommandSuggestion[] = this.extractSection(response, 'Fixes')
        .split('\n')
        .map(fix => {
