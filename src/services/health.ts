...
async function checkExternalAPIs(): Promise<HealthCheckResult> {
  ...
  try {
    // Check Website API
    const websiteApiResponse = await axios.get(`${env.WEBSITE_API_URL}/health`, {
      timeout: 5000,
    });
    
    if (websiteApiResponse.status !== 200) {
      throw new Error(`Website API returned status ${websiteApiResponse.status}`);
    }
...
export async function runHealthChecks(): Promise<Record<string, HealthCheckResult>> {
  const [dbHealth, redisHealth, apiHealth, systemHealth] = await Promise.allSettled([
    checkDatabase(),
    checkRedis(),
    checkExternalAPIs(),
    checkSystem(),
