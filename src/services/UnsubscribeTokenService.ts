...
class UnsubscribeTokenService {
  ...
  static generateToken(userId: string, reportType: string): string {
    try {
      ...
      return Buffer.from(`${payload}${this.TOKEN_SEPARATOR}${hmac}`).toString('base64');
    } catch (error) {
   ...
   */
  static verifyToken(token: string, userId: string, reportType: string): boolean {
    try {
      // Decode token
      const decodedToken = Buffer.from(token, 'base64').toString();
      const [version, tokenUserId, tokenReportType, timestamp, hmac] = decodedToken.split(this.TOKEN_SEPARATOR);
      ...
      const expectedHmac = createHmac('sha256', config.security.unsubscribeSecret)
        .update(payload)
        .digest('hex');
