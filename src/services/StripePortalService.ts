...
class StripePortalService {
  ...
  async createPortalSession(userId: string): Promise<string> {
    try {
      ...
      if (!user?.stripeCustomerId) {
        throw new AppError('User not found or no billing setup', 'INVALID_USER', 400);
      }
      ...
      throw new AppError('Could not create billing portal session', 'PORTAL_ERROR', 500);
    }
  }
  ...
  private async initializePortalConfig(): Promise<void> {
    try {
      ...
      const businessProfile = await this.stripe.billingPortal.configurations.create({
        business_profile: {
