...
class PaymentMonitoringService {
  ...
  async monitorPaymentPatterns(userId: string, amount: number): Promise<void> {
    try {
      ...
      if (recentPayments.length >= 5) {
        const timeDiff = recentPayments[0].createdAt.getTime() - 
      ...
      const averageAmount = recentPayments.reduce((sum, payment) => 
        sum + payment.amount, 0) / recentPayments.length;
  ...
  async assessChargebackRisk(userId: string, amount: number): Promise<{
    ...
    try {
      ...
      if (accountAge < 30 * 24 * 60 * 60 * 1000) { // Less than 30 days
        riskScore += 30;
      }

      // Factor 2: Payment history
      const successfulPayments = user.paymentHistory.filter(p => p.status === 'SUCCESS').length;
      ...
      const averageTransactionAmount = user.paymentHistory.reduce((sum, payment) => 
        sum + payment.amount, 0) / user.paymentHistory.length || 0;
