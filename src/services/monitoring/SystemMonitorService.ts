...
class SystemMonitorService {
  ...
  private async getProcessMetricsDarwin(
    pid: number
    ...
    try {
      ...
      logger.error(
        `Failed to get Darwin process metrics for PID ${pid}:`,
        error
      );
      return null;
    }
  ...
  private async getProcessMetricsLinux(
    pid: number
  ): Promise<ProcessMetrics | null> {
    try {
      ...
      logger.error(
        `Failed to get Linux process metrics for PID ${pid}:`,
        error
      );
      return null;
    }
