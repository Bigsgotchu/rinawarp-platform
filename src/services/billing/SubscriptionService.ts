...
class SubscriptionService {
  ...
  async getSubscriptionDetails(userId: string): Promise<SubscriptionDetails> {
    try {
      ...
      const usage = {
        commands: {
          used:
            metrics.find(m => m.metricType === 'commands_executed')?.count || 0,
          limit: PLAN_FEATURES[user.currentPlan].maxCommands,
        },
        workflows: {
          used:
            metrics.find(m => m.metricType === 'workflows_created')?.count || 0,
  ...
  async checkUsageLimit(
    ...
    try {
      ...
      const limit =
        metricType === 'commands' ? limits.maxCommands : limits.maxWorkflows;
