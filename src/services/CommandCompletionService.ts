...
class CommandCompletionService {
  ...
  async getCompletions(context: CompletionContext): Promise<Completion[]> {
    try {
      ...
      const workspaceContext = context.workspacePath
        ? await WorkspaceContextService.getWorkspaceContext(context.workspacePath)
        : undefined;
      ...
      if (context.workspacePath) {
        const workflowSuggestions = await WorkflowLearningService.suggestNextCommands(
          parts.join(' '),
  ...
  private async getArgumentCompletions(
    ...
    switch (command) {
      case 'git':
        completions.push(...this.getGitArgCompletions(currentArg, previousArgs, context));
        break;
      case 'docker':
        completions.push(...this.getDockerArgCompletions(currentArg, previousArgs, context));
        break;
      case 'npm':
      case 'yarn':
        completions.push(...this.getPackageArgCompletions(command, currentArg, previousArgs, context));
        break;
    }
  ...
  private getGitArgCompletions(
    ...
    if (!previousArgs.length) {
      ...
      const gitCommands = ['add', 'commit', 'push', 'pull', 'checkout', 'merge'];
      completions.push(
        ...gitCommands
    ...
    if (!previousArgs.length) {
      ...
      switch (lastArg) {
        case 'run':
          ...
          completions.push(
            ...Object.entries(context.package.scripts || {}).map(([name, script]) => ({
              command: name,
