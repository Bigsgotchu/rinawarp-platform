...
class WorkspaceContextService {
  ...
  private async getGitContext(workspacePath: string): Promise<GitContext | undefined> {
    try {
      ...
      const [branch, status, remotes, lastCommit] = results.map(([_, output]) => output);
  ...
  private async getDockerContext(workspacePath: string): Promise<DockerContext | undefined> {
    try {
      const commands = {
        containers: 'docker ps --format "{{.ID}}\\t{{.Names}}\\t{{.Status}}\\t{{.Image}}"',
      ...
      try {
        ...
        const services = composeContent.match(/^  \w+:/gm)?.map(s => s.slice(2, -1)) || [];
        compose = {
  ...
  private async getPackageContext(workspacePath: string): Promise<PackageContext | undefined> {
    try {
  ...
  private async getEnvContext(workspacePath: string): Promise<EnvContext | undefined> {
    try {
    ...
    try {
      ...
      if (context.git) {
        if (context.git.status.modified.length > 0) {
          suggestions.push('You have modified files. Consider committing your changes.');
        }
      ...
      if (context.docker?.containers.length === 0 && context.docker?.compose) {
        suggestions.push('Docker Compose configuration exists but no containers are running.');
      }
      ...
      if (context.package) {
        ...
        try {
          ...
          suggestions.push('package-lock.json is missing. Run npm install to generate it.');
        }
      }

      // Environment suggestions
      if (context.env?.files.length === 0) {
        suggestions.push('No .env file found. Consider adding one for environment variables.');
      }
  ...
  async validateConfiguration(workspacePath: string): Promise<{ [key: string]: boolean }> {
    try {
      const context = await this.getWorkspaceContext(workspacePath);
      ...
      // Git configuration
      if (context.git) {
        const { stdout: gitConfig } = await execAsync('git config --list', { cwd: workspacePath });
        validation.gitUser = gitConfig.includes('user.name') && gitConfig.includes('user.email');
      ...
      if (context.package) {
        ...
        validation.hasLockFile = context.package.manager === 'npm'
  ...
  private async fileExists(workspacePath: string, file: string): Promise<boolean> {
    try {
