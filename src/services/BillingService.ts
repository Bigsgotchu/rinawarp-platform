...
class BillingService {
  ...
  private readonly USAGE_RATES = {
    [SubscriptionPlan.BASIC]: {
      ...
      additionalWorkflows: 1.00, // $1.00 per workflow
    },
    [SubscriptionPlan.PRO]: {
      additionalCommands: 0.005, // $0.005 per command
      additionalWorkflows: 0.50, // $0.50 per workflow
    },
  ...
  async generateInvoice(userId: string, billingCycle: BillingCycle): Promise<void> {
    try {
  ...
  async calculateUsageCharges(
    ...
    try {
      ...
      const commandOverage = Math.max(0, usage['commands_executed'] - limits.commands);
      if (commandOverage > 0) {
        charges.push({
      ...
      const workflowOverage = Math.max(0, usage['workflows_created'] - limits.workflows);
    ...
    await db.user.update({
      ...
      data: {
        scheduledDowngrade: {
          ...
          effectiveDate: new Date(Date.now() + gracePeriod * 24 * 60 * 60 * 1000),
        },
      },
    });
  }
