...
class CommandChainService {
  ...
  async executeChain(
    ...
    try {
      ...
      if (chain.context?.workspace && workspacePath) {
        ...
        const validation = await WorkspaceContextService.validateConfiguration(
          workspacePath
        );
      ...
      for (let i = 0; i < chain.steps.length; i++) {
        ...
        do {
          try {
            ...
            logger.warn(`Retrying step ${step.id}, attempt ${retries}/${maxRetries}`);
  ...
  async suggestChain(
    ...
    try {
      ...
      const chainStructure = await AIService.analyzeCommand(
        commands[0],
        {
