...
interface UsageMetrics {
  ...
  usageByPlan: Record<SubscriptionPlan, {
    commands: number;
    workflows: number;
...
class AnalyticsService {
    ...
    try {
      const [
        ...
        db.paymentHistory.aggregate({
          where: {
            createdAt: {
              gte: new Date(range.start.getTime() - (range.end.getTime() - range.start.getTime())),
              lt: range.start,
            },
            status: 'PAID',
          },
      ...
      const revenueChangePercentage = previousRevenue ? (revenueChange / previousRevenue) * 100 : 0;
  ...
  async getSubscriptionMetrics(date: Date = new Date()): Promise<SubscriptionMetrics> {
    try {
      ...
      const activeSubscriptions = subscriptionCounts.find(
        group => group.subscriptionStatus === 'active'
      )?._count || 0;

      const planDist = planDistribution.reduce((acc, group) => {
  ...
  private async calculateRecurringRevenue(date: Date): Promise<{
    ...
    const mrr = subscriptions.reduce((total, sub) => {
      return total + (sub._count * planPrices[sub.currentPlan as SubscriptionPlan]);
    }, 0);
  ...
  private async calculateChurnRate(date: Date): Promise<{
    ...
    return {
      churnRate: totalUsersLastMonth ? (churnedUsers / totalUsersLastMonth) * 100 : 0,
      churnedUsers,
    };
  }

  private async calculateUsageByPlan(range: DateRange): Promise<Record<SubscriptionPlan, {
    commands: number;
    workflows: number;
    averagePerUser: number;
  }>> {
    ...
    const planUsage = {} as Record<SubscriptionPlan, {
      commands: number;
      workflows: number;
      users: Set<string>;
    }>;
    ...
    return Object.entries(planUsage).reduce((acc, [plan, data]) => {
      acc[plan as SubscriptionPlan] = {
        commands: data.commands,
        workflows: data.workflows,
        averagePerUser: data.users.size ? 
          (data.commands + data.workflows) / data.users.size : 0,
      };
      return acc;
  ...
  private async calculateCustomerLifetimeValue(): Promise<number> {
    ...
    return totalCustomers ? (totalRevenue._sum.amount || 0) / totalCustomers : 0;
  }
