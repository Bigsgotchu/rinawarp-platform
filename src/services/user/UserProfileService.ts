...
class UserProfileService {
  ...
  async updateFromCommand(
    ...
    cat.usageCount++;
    cat.successRate =
      (cat.successRate * (cat.usageCount - 1) +
  ...
  private updateSkillLevel(
    ...
    profile.skillLevel.categoryLevels[category] = Math.min(
      ...
      Math.max(
        0,
        currentLevel +
          learningFactor * complexityFactor * profile.skillLevel.learningRate
      )
    );
    ...
    profile.skillLevel.overall =
      categoryLevels.reduce((sum, level) => sum + level, 0) /
      categoryLevels.length;

    // Update learning rate
    const timeSinceLastUpdate =
      Date.now() - profile.skillLevel.lastUpdated.getTime();
    const daysSinceLastUpdate = timeSinceLastUpdate / (1000 * 60 * 60 * 24);
    profile.skillLevel.learningRate = Math.max(
      0.5,
      1 - daysSinceLastUpdate * 0.1
    );
  ...
  async getProfileStats(userId: string): Promise<any> {
    ...
    return {
      commandStats: {
        ...
        successRate:
          profile.metrics.successfulCommands / profile.metrics.totalCommands,
        categoryBreakdown: Object.values(profile.metrics.categories).map(
          cat => ({
            name: cat.name,
