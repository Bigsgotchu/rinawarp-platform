...
class EmailMonitoringService {
   ...
   */
  async checkDeliveryHealth(timeWindowMs: number = 24 * 60 * 60 * 1000) { // Default 24h
    ...
    try {
      ...
      const failures = stats.find(stat => stat.status === 'FAILURE')?._count || 0;
      ...
      return {
        ...
        stats: {
          ...
          failures,
          failureRate,
          averageDeliveryTime: stats.find(stat => stat.status === 'SUCCESS')?._avg.deliveryTimeMs
        }
      };
