...
class WorkflowLearningService {
  ...
  private async updateCommandSequence(
    ...
    sequence.push({
      ...
      context: context
        ? {
            directory: context.currentDirectory,
            gitBranch: context.git?.branch,
            dockerContainers: context.docker?.containers.map(
              (c: any) => c.name
            ),
            packageManager: context.package?.manager,
          }
    ...
    await this.redis.set(this.COMMAND_SEQUENCE_KEY, JSON.stringify(sequence));
  }
  ...
  private extractPatternContext(
    ...
    const contexts = sequence.map(step => step.context).filter(Boolean);
  ...
  private calculatePatternScore(
    ...
    if (
      pattern.contexts.projectType === this.inferProjectTypeFromContext(context)
    ) {
      score *= 1.5;
    }

    if (
      pattern.contexts.techStack?.some(
        tech =>
