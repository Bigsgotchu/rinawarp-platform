...
export function validateErrorAnalysis(
  ...
  if (!workspacePath) {
    return next(new AppError('Workspace path is required', 'INVALID_INPUT', 400));
  }
...
export function validateRecoveryAttempt(
  ...
  if (!originalCommand) {
    return next(new AppError('Original command is required', 'INVALID_INPUT', 400));
  }
  ...
  if (!recoveryCommand) {
    return next(new AppError('Recovery command is required', 'INVALID_INPUT', 400));
  }
