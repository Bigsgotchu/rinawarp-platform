...
export const validateBilling = {
  addPaymentMethod: (req: Request, res: Response, next: NextFunction) => {
    ...
    if (
      ...
      return next(
        new AppError('Invalid payment method ID format', 'INVALID_INPUT', 400)
      );
    }
  ...
  processPayment: (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!currency || currency !== 'usd') {
      return next(
        new AppError('Invalid or unsupported currency', 'INVALID_INPUT', 400)
      );
    }

    if (!paymentMethodId || typeof paymentMethodId !== 'string') {
      return next(
        new AppError('Invalid payment method ID', 'INVALID_INPUT', 400)
      );
    }
  ...
  validateInvoice: (req: Request, res: Response, next: NextFunction) => {
    ...
    for (const item of items) {
      if (
        ...
        return next(
          new AppError('Invalid invoice item format', 'INVALID_INPUT', 400)
        );
      }
    }
    ...
    if (!customerId) {
      return next(
        new AppError('Customer ID is required', 'INVALID_INPUT', 400)
      );
    }
  ...
  validateCreditCardInfo: (req: Request, res: Response, next: NextFunction) => {
    const { number, expiryMonth, expiryYear, cvc, name } = req.body;

    // Validate card number using Luhn algorithm
    if (!validateCreditCard(number)) {
      return next(
        new AppError('Invalid credit card number', 'INVALID_INPUT', 400)
      );
    }
    ...
    if (!name || typeof name !== 'string' || name.length < 2) {
      return next(
        new AppError('Invalid cardholder name', 'INVALID_INPUT', 400)
      );
    }
  ...
  validateBillingAddress: (req: Request, res: Response, next: NextFunction) => {
    const { line1, city, country, postalCode } = req.body;
