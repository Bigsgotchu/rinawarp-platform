...
export const validateSubscription = {
  create: (req: AuthRequest, res: Response, next: NextFunction) => {
    ...
    if (!plan || !Object.values(SubscriptionPlan).includes(plan)) {
      return next(
        new AppError('Invalid subscription plan', 'INVALID_INPUT', 400)
      );
    }
    ...
    if (!paymentMethodId || typeof paymentMethodId !== 'string') {
      return next(
        new AppError('Payment method ID is required', 'INVALID_INPUT', 400)
      );
    }
  ...
  validateUsage: (req: AuthRequest, res: Response, next: NextFunction) => {
  ...
  validateBillingCycle: (
    ...
    if (start >= end) {
      return next(
        new AppError('Start date must be before end date', 'INVALID_INPUT', 400)
      );
    }

    if (start < new Date(Date.now() - 365 * 24 * 60 * 60 * 1000)) {
      return next(
        new AppError('Start date too far in the past', 'INVALID_INPUT', 400)
      );
    }

    if (end > new Date(Date.now() + 365 * 24 * 60 * 60 * 1000)) {
      return next(
        new AppError('End date too far in the future', 'INVALID_INPUT', 400)
      );
    }
  ...
  validateTrialPeriod: (
    ...
    if (!trialDays || typeof trialDays !== 'number') {
      return next(
        new AppError('Trial period is required', 'INVALID_INPUT', 400)
      );
    }
  ...
  validateCancellation: (
    ...
    if (!reason || !validReasons.includes(reason)) {
      return next(
        new AppError('Invalid cancellation reason', 'INVALID_INPUT', 400)
      );
    }

    if (reason === 'other' && (!feedback || typeof feedback !== 'string')) {
      return next(
        new AppError('Feedback required for other reason', 'INVALID_INPUT', 400)
      );
    }

    if (feedback && typeof feedback !== 'string') {
      return next(
        new AppError('Invalid feedback format', 'INVALID_INPUT', 400)
      );
    }
  ...
  validateProrationMode: (
    ...
    const validModes = ['create_prorations', 'none', 'always_invoice'];
