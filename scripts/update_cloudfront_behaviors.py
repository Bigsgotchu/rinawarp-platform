...
def update_distribution_behaviors(dist_id):
    """Update a CloudFront distribution's cache behaviors."""
    ...
    # Update origin path
    if '/api' in config['Origins']['Items'][0]['OriginPath']:
        config['Origins']['Items'][0]['OriginPath'] = '/api'
    elif '/docs' in config['Origins']['Items'][0]['OriginPath']:
...
def main():
    ...
    for dist in distributions:
        ...
        success = update_distribution_behaviors(dist_id)
        if success:
